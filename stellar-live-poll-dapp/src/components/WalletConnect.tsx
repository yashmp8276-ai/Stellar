    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
      ...
      {address && !networkOk && (
        <div className="network-warning">
          ...
          <div>
            <div style={{ fontWeight: 700, marginBottom: 2 }}>Network Mismatch</div>
            <div style={{ fontSize: '0.8rem', opacity: 0.85 }}>
              Wallet is on <strong>{currentNetwork || 'Unknown'}</strong>.
              Switch to <strong style={{ color: '#fff' }}>TESTNET</strong> in your wallet settings.
            </div>
          </div>
        </div>
      )}
      ...
      <div className="glass-card wallet-panel">
        <div className="wallet-info">
          <h2>🔗 Wallet</h2>
          <p>{address ? 'Connected to Stellar Testnet' : 'Connect any Stellar wallet to vote.'}</p>
        </div>
          ...
          {address ? (
            <>
              <div className="address-chip">
                <span style={{ fontFamily: 'ui-monospace, monospace', fontSize: '0.825rem', color: '#818cf8' }}>
                  {shortAddr}
                </span>
                <button id="copy-addr-btn" className="copy-btn" onClick={copyAddress} title="Copy">
                  {copySuccess ? '✅' : '📋'}
                </button>
              </div>
