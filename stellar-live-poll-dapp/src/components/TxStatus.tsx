  ...
  return (
    <div className="glass-card tx-status-card">
      ...
      <div className="tx-steps">
        {stepsList.map((s, idx) => {
          // If we have an error at or after this step, render it as error.
          // In a simple approach, we mark the step that failed.
          // Let's assume the current/last attempted step failed if isError is true.
          const isDone = !isError && idx < currentIdx;
          const isActive = !isError && idx === currentIdx;
          const stepFailed = isError && idx === currentIdx; // failed here
